...
export function fitSpoken(text, maxChars) {
...
Contraintes :
- text : français, tutoiement, 1 à 2 phrases, max ${Number(settings.quotes?.maxCharsFor10s || 140)} caractères, à dire en 10 secondes, rythme oral
...
JSON :
{
  ...
  const text = fitSpoken(String(data.text || "").replace(/\s+/g, " ").trim(), maxChars);
...
export function clipPrompt({ visualPrompt, spoken }) {
  return [
